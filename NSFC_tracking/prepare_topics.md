...
| :---: | :---: | :---: | :---: | :---: | :---: |
| 车联网复杂移动环境感知与建模理论（2018-2022，重点） | 郑侃 | 北京邮电大学 | [Link](https://scholar.google.com/citations?hl=en&user=IX_id5UAAAAJ&view_op=list_works&sortby=pubdate) | [Link](https://ieeexplore.ieee.org/author/37276693000) | - |
- **项目摘要：** 车辆数目的急剧增加导致交通事故频发、交通效率下降，面向5G应用的车联网技术成为提升交通安全和效率的重要手段，而复杂移动环境感知与建模研究是推动车联网关键技术发展的核心动力。因此，本课题首先探索高效的复杂移动环境感知方法，获取车辆的动态交通视图和频谱地图；然后，基于感知的复杂移动环境信息，从微观和宏观两个角度对复杂移动行为进行深度剖析与精确建模，并揭示其潜在的规律；最后，分析面向复杂移动行为的车联网拓扑结构，挖掘网络通信性能的内在特征，实现网络覆盖性能的优化和资源的高效利用。力求通过该项目的探索，为面向5G应用的车联网接入与可靠传输、网络资源调度与车辆移动优化奠定基础。
- **结题摘要：** 车辆数目的急剧增加导致交通事故频发、交通效率下降，面向下一代通信网络应用的车联网技术成为提升交通安全和效率的重要手段，而复杂移动环境感知与建模研究是推动车联网关键技术发展的核心动力。因此，本课题首先探索高效的复杂移动环境感知方法，提出了高精度的多目标检测、深度定位和频谱感知方法，并进行数据特征的深度分析与处理，以获取车辆的动态交通视图和频谱地图；然后，基于感知的复杂移动环境信息，从微观和宏观两个角度对复杂移动行为进行深度剖析，提出了精确的宏观移动行为和微观车辆轨迹模型，并揭示其潜在的规律，基于此提出满足多种需求的智能自动驾驶方法；进而，分析面向复杂移动行为的车联网拓扑结构，挖掘网络通信性能的内在特征，提出多种V2X网络覆盖性能的优化和资源的高效利用方法；最后，从复杂移动行为规律、网络拓扑结构特征以及资源高效利用的角度，建立了车联网移动安全架构，提出满足安全攻击、信息共享以及特殊业务需求的高安全策略。力求通过该项目的探索，为面向未来应用的车联网接入与可靠传输、网络资源调度与车辆移动优化、车联网信息安全共享等奠定基础。
...
| Program | Leader | Affiliation | Google scholar | IEEE | Github |
| :---: | :---: | :---: | :---: | :---: | :---: |
| 车联网架构下高效的信息分发安全关键技术研究（2019-2022，青年） | 夏莹杰 | 浙江大学 | [Link](https://scholar.google.com/citations?user=lWN9EjkAAAAJ&hl=en&oi=ao) | [Link](https://ieeexplore.ieee.org/author/37407393000) | - |
- **项目摘要：** 新兴的车联网具有广阔的应用前景，信息分发是众多车联网应用的核心支撑技术，对所有车联网提供的交通安全、交通管理、信息娱乐等信息服务具有重要意义。为了保障信息分发的安全性和高效性，本项目拟设计信息分发的安全模型，研究基于信息、车流、车辆等多维特征的目标路侧单元(roadside unit, RSU)筛选策略和群集属性自动生成方法，采用属性基加密的广播加密，保证只有被授权的车辆能够访问信息；考虑复杂交通环境下面对大量实时消息的典型信息处理场景，研究车辆侧自适应智能决策解密机制，保证紧急信息的及时获知；针对由于车辆的快速移动使得跨RSU切换中车辆获得密文信息的完整性问题，研究基于车辆轨迹的RSU切换预测和基于代理重加密的密文安全转换，保证信息分发的可靠性。
- **结题摘要：** 车联网具有广阔的应用前景，信息分发是众多车联网应用的核心支撑技术,如何保证信息分发的安全和高效成为影响车联网发展的关键研究工作。基于对车联网环境下信息分发安全问题的分析总结，本项目拟定的总体研究目标：研究安全的信息分发机制，基于消息类型、影响范围、车流信息等特征来选择分发范围、分发方式，保证消息有效、安全地分发至影响区域内相关车辆；研究跨RSU信任域的信息安全传输机制，提高分发信息的准确性，保证车联网信息能够为高速移动的车辆提供可靠服务；研究面向车联网信息分发的高效机制，使得信息能够在路侧单元与车之间进行安全、高效地分发与传输，平衡安全和性能需求，保证高质量的车联网信息分发与处理。.在国家基金的支持下，本项目具体完成了以下三个方面，具体来说（1）安全的信息分发：利用属性基加密的策略融合方法，灵活地制定了满足精准访问需求的细粒度访问控制策略；基于多权威机构共治的属性基签名，实现车联网消息安全认证的同时保护数据发送车辆的身份隐私（2）可靠的信息分发：基于车联网信息的时空相关性提高信息分发质量；构建基于区块链搭建安全的车联网信息分发平台和基于层次分析法设计混合信誉评估系统，对信息分发实体进行可靠地信誉评估，从而保证跨RSU信息分发的安全性（3）高效的信息分发：基于轻量级的Boneh-Lynn-Shacham签名和群签名，设计了低计算开销的车辆雾节点批量匿名认证方案；基于区块链和改进的密文策略属性基签名进行跨域数据细粒度的安全共享，采用布隆过滤器和链上智能合约进行跨域访问验证，实现高效的跨域数据请求验证和获取。
...
- Yu, Kan, Zhiyong Feng, Dong Li, and Jiguo Yu. "Secure-ISAC: Secure V2X Communication: An Integrated Sensing and Communication Perspective." arXiv preprint arXiv:2312.01720 (2023). [[paper]](https://arxiv.org/abs/2312.01720)
  - 物理层安全
- N. Huang et al., "Mobile Edge Computing aided Integrated Sensing and Communication with Short-Packet Transmissions," in IEEE Transactions on Wireless Communications, doi: 10.1109/TWC.2023.3344479. [[paper]](https://ieeexplore.ieee.org/document/10375321)
